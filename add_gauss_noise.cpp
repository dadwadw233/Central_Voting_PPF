...
void AddGaussNoise::addGaussNoise(pcl::PointCloud<pcl::PointXYZ> &cloud_out) {
  ...
  boost::normal_distribution<> nd(
      m_mu, m_sigma);  //定义正态分布，均值为mu，标准差为sigma
  boost::variate_generator<boost::mt19937 &, boost::normal_distribution<>>
      gauss_noise(zgy, nd);  //生成高斯噪声
  ...
  std::cout << "gauss size: " << cloud_gauss.points.size()
            << " scene size: " << m_cloud_in.points.size() << std::endl;
  cloud_out = m_cloud_in +
              cloud_gauss;  //将原始点云与噪声点云合并，得到添加高斯噪声后的点云
}
